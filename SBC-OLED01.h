...
class SBS_OLED01 {
  ...
  public:
    ...
    static constexpr int BYTES_PER_SEG = HEIGHT / 8; // number of bytes per displayed column of pixels
    ...
    // Create, start and prepare a screen upload feed. Caller must .stop() it.
    static USI_TWI_Master feed() {
      return master().transmit(PAYLOAD_DATA);
    }
